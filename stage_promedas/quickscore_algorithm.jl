# function fun_quickscoreW(previn, pfmin, pfminneg)
# Wim Wiegerinck version 8/9/2023
# inputs:
# previn               prevalences
...
pfpluspd = pfplus .* prevend  # numerator in heckerman (12)  p(F+,F-),P(F+,F-|d_1=1)p(d_1=1),..., P(F+,F-|d_n=1)p(d_n=1)
# the 2nd until (n+1)nd element are the terms for d_1=1,...,d_n=1
# and for all the elements, normalization is done with p(F+,F-).
posterior = (pfpluspd[2:(n+1)] ./ pfpluspd[1])'

# which is the most important output: posterior = P(d_1=1|F+,F-),... P(d_n=1|F+,F-)
